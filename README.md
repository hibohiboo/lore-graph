...
- **LLM Adapter** — OpenAI API またはローカルの Ollama を使い、対話生成と構造化情報の抽出を担当します。
- **Lore Engine** — システムの中核。抽出されたエンティティ・関係を受け取り、世界モデルを管理し、矛盾を検出して、クエリインターフェースを提供します。
- **NPC Mind** — NPC ごとの信念状態・知識範囲・対話パーソナリティのモデリングを担当します。
- **Graph DB** — 世界の正規状態をプロパティグラフとして保持します。
